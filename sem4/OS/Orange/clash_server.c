...
#define STUDENT_SRN "PES2UG24CS157"
...
bool spend_elixir(int cost) {
    ...
    // Lock the elixir pool and check the FULL cost atomically.
    // If insufficient, release immediately — no partial hold (Hold-and-Wait Prevention).
...
void *card_spawner(void *id_ptr) {
    ...
    while (1) {
        ...
        if (spend_elixir(elixir_cost)) {
            ...
            // Wait for a free slot (blocks if arena is full)
            sem_wait(&arena_empty_slots);

            // Critical section: write into arena array
            pthread_mutex_lock(&arena_mutex);
            arena[deploy_idx] = elixir_cost;
